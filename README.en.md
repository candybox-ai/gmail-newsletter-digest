...
This skill turns your Gmail newsletter subscriptions into a morning and evening digest worth reading, so you don't have to open every message one by one or miss what matters.
...
People whose inbox receives a lot of newsletters.
...
On first use, complete Setup in order (confirm each step before moving on):
...
4. **Choose the language** — lock the language used in the digest body
5. **Set delivery** — delivered in chat by default; you can also specify a recipient address

## Requirements

- **Gmail read and write access**: read subscription mail from the chosen Gmail account; sending permission is additionally required if you enable email delivery
- Schedules run in the timezone you set during Setup
